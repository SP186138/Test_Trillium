...
:: +---------------------------------------------------------------------------+
:: | File:     TSSSwitzerlandMatch.cmd                                         |
:: | Purpose:  Match 1CP Switzerland data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |   602 - combining link files & resolve transitivity                       |
...
:: |  05-Aug-2013  sheldon    New multi-match design.                          |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\che\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\che\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_CHE.DAT" (
      @echo ERROR: [%DATE%%TIME%] CHE reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_CHE.DAT" (
      @echo CHE cleanse output file is missing.  No CHE matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_CHE.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_CHE.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_CHE (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\ch_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\ch_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\ch_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\ch_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\ch_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\ch_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\ch_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\ch_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_ch_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_ch_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\ch_tranfrmr_p9.dat"  2>nul
   @del /Q "%TS_DATA%\ch_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\ch_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\ch_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\ch_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\ch_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSwitzerlandMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSwitzerlandMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %CHE_WRK%\data\TSS_MATCH_OUTPUT_CHE.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_CHE.DAT
