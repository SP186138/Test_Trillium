...
:: | File:     TSSItalyCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP Italy data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\ita\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\ita\cleanse
   @set TS_WORKAREA=%ITA_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_ITA.DAT" (
      @echo ITA cleanse input file is missing.  No ITA cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_ITA.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_ITA.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_ITA.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_ITA.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ittranfrmr_p1.stx"  "%TS_LOGS%\p1_ittranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\itcusparse_p2.stx"  "%TS_LOGS%\p2_itcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\itsrtforpm_p3.stx"  "%TS_LOGS%\p3_itsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\itpmatch"  "%TS_SETTINGS%\itpmatch_p4.stx"    "%TS_LOGS%\p4_itpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\itdrtrans_p5.stx"   "%TS_LOGS%\p5_itdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\itlatlon_p9.stx"    "%TS_LOGS%\p9_itlatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\itwinkey_p6.stx"    "%TS_LOGS%\p6_itwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ittranfrmr_p7.stx"  "%TS_LOGS%\p7_ittranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_ITA.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_ITA.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ittranfrmr_p8.stx"  "%TS_LOGS%\p8_ittranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSItalyCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSItalyCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %ITA_WRK%\data\TSS_WINKEY_OUTPUT_ITA.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_ITA.DAT
)
