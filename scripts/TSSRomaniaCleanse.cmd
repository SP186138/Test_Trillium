...
:: +---------------------------------------------------------------------------+
:: | File:     TSSRomaniaCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Romania data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\rou\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\rou\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_ROU.DAT" (
      @echo ROU cleanse input file is missing.  No ROU cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_ROU.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_ROU.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_ROU.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_ROU.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\rotranfrmr_p1.stx"  "%TS_LOGS%\p1_rotranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\rodrtrans_p2.stx"   "%TS_LOGS%\p2_rodrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\rotranfrmr_p3.stx"  "%TS_LOGS%\p3_rotranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_ROU.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_ROU.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\rotranfrmr_p4.stx"  "%TS_LOGS%\p4_rotranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSRomaniaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSRomaniaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %ROU_WRK%\data\TSS_WINKEY_OUTPUT_ROU.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_ROU.DAT
)
