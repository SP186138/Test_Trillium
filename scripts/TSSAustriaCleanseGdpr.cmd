...
:: | File:     TSSAustriaCleanse.cmd                                           |
:: | Purpose:  Cleanse 1CP Austria data.                                       |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\aut\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\aut\cleanse
   @set TS_WORKAREA=%AUT_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_AUT.DAT" (
      @echo AUT cleanse input file is missing.  No AUT cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_AUT.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_AUT.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_AUT.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_AUT.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\attranfrmr_p1.stx"  "%TS_LOGS%\p1_attranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\atcusparse_p2.stx"  "%TS_LOGS%\p2_atcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\atsrtforpm_p3.stx"  "%TS_LOGS%\p3_atsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gapmatch"  "%TS_SETTINGS%\atpmatch_p4.stx"    "%TS_LOGS%\p4_atpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\atdrtrans_p5.stx"   "%TS_LOGS%\p5_atdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\atlatlon_p9.stx"    "%TS_LOGS%\p9_atlatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\atwinkey_p6.stx"    "%TS_LOGS%\p6_atwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\attranfrmr_p7.stx"  "%TS_LOGS%\p7_attranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_AUT.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_AUT.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\attranfrmr_p8.stx"  "%TS_LOGS%\p8_tranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSAustriaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSAustriaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
    ...
    @if "%TSS_PROCESS%" == "i2" (
copy %AUT_WRK%\data\TSS_WINKEY_OUTPUT_AUT.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_AUT.DAT
    )
