...
:: | File:     TSSSwitzerlandCleanse.cmd                                       |
:: | Purpose:  Cleanse 1CP Switzerland data.                                   |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |                                                                           |
:: | Usage:                                                                    |
:: |   > TSSSwitzerlandCleanse [workarea]                                      |
:: |     where:                                                                |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\che\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\che\cleanse
   @set TS_WORKAREA=%CHE_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_CHE.DAT" (
      @echo CHE cleanse input file is missing.  No CHE cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_CHE.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_CHE.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_CHE.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_CHE.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\chtranfrmr_p1.stx"  "%TS_LOGS%\p1_chtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\chcusparse_p2.stx"  "%TS_LOGS%\p2_chcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\chsrtforpm_p3.stx"  "%TS_LOGS%\p3_chsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tqpmatch"  "%TS_SETTINGS%\chpmatch_p4.stx"    "%TS_LOGS%\p4_chpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\chdrtrans_p5.stx"   "%TS_LOGS%\p5_chdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\chlatlon_p9.stx"    "%TS_LOGS%\p9_chlatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\chwinkey_p6.stx"    "%TS_LOGS%\p6_chwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\chtranfrmr_p7.stx"  "%TS_LOGS%\p7_chtranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_CHE.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_CHE.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\chtranfrmr_p8.stx"  "%TS_LOGS%\p8_chtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSwitzerlandCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSwitzerlandCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %CHE_WRK%\data\TSS_WINKEY_OUTPUT_CHE.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_CHE.DAT
)
