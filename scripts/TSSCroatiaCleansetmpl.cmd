:: +---------------------------------------------------------------------------+
:: | File:     TSSCroatiaCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Croatia data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\hrv\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\hrv\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_HRV.DAT" (
      @echo HRV cleanse input file is missing.  No HRV cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_HRV.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_HRV.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_HRV.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_HRV.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hrtranfrmr_p1.stx"  "%TS_LOGS%\p1_hrtranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hrdrtrans_p2.stx"   "%TS_LOGS%\p2_hrdrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hrtranfrmr_p3.stx"  "%TS_LOGS%\p3_hrtranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_HRV.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_HRV.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hrtranfrmr_p4.stx"  "%TS_LOGS%\p4_hrtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCroatiaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCroatiaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %HRV_WRK%\data\TSS_WINKEY_OUTPUT_HRV.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_HRV.DAT
)
