...
:: +---------------------------------------------------------------------------+
:: | File:     TSSUSAMatch.cmd                                                 |
:: | Purpose:  Match 1CP United States data.                                   |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |  09-May-2012  sheldon    Phase-0 release.                                 |
:: |  10-May-2012  sheldon    Changes to support new "BADKEY" processing as    |
:: |                          replacement for tie-breaker field.               |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\usa\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\usa\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_USA.DAT" (
      @echo ERROR: [%DATE%%TIME%] USA reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_USA.DAT" (
      @echo USA cleanse output file is missing.  No USA matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_USA.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_USA.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_USA (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\us_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\us_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\us_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\us_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\us_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\us_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\us_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\us_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_us_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_us_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\us_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\us_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\us_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\us_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\us_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\us_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSUSAMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSUSAMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %US_WRK%\data\TSS_MATCH_OUTPUT_USA.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_USA.DAT
