...
:: | File:     TSSPortugalCleanse.cmd                                          |
:: | Purpose:  Cleanse 1CP Portugal data.                                      |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\prt\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\prt\cleanse
   @set TS_WORKAREA=%PRT_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_PRT.DAT" (
      @echo PRT cleanse input file is missing.  No PRT cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_PRT.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_PRT.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_PRT.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_PRT.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\pttranfrmr_p1.stx"  "%TS_LOGS%\p1_pttranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\ptcusparse_p2.stx"  "%TS_LOGS%\p2_ptcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\ptsrtforpm_p3.stx"  "%TS_LOGS%\p3_ptsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tqpmatch"  "%TS_SETTINGS%\ptpmatch_p4.stx"    "%TS_LOGS%\p4_ptpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ptdrtrans_p5.stx"   "%TS_LOGS%\p5_ptdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\ptlatlon_p9.stx"    "%TS_LOGS%\p9_ptlatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\ptwinkey_p6.stx"    "%TS_LOGS%\p6_ptwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\pttranfrmr_p7.stx"  "%TS_LOGS%\p7_pttranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_PRT.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_PRT.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\pttranfrmr_p8.stx"  "%TS_LOGS%\p8_tranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSPortugalCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSPortugalCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %PRT_WRK%\data\TSS_WINKEY_OUTPUT_PRT.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_PRT.DAT
)
