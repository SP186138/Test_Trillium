...
:: +---------------------------------------------------------------------------+
:: | File:     TSSHongKongMatch.cmd                                            |
:: | Purpose:  Match 1CP Hong Kong data.                                       |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\hkg\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\hkg\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_HKG.DAT" (
      @echo ERROR: [%DATE%%TIME%] HKG reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_HKG.DAT" (
      @echo HKG cleanse output file is missing.  No HKG matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_HKG.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_HKG.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_HKG (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\hk_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\hk_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\hk_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\hk_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\hk_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\hk_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\hk_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\hk_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_hk_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_hk_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\hk_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\hk_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\hk_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\hk_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\hk_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\hk_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHongKongMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHongKongMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %HKG_WRK%\data\TSS_MATCH_OUTPUT_HKG.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_HKG.DAT
