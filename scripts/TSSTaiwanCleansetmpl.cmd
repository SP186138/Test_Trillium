:: +---------------------------------------------------------------------------+
:: | File:     TSSTaiwanCleanse.cmd                                            |
:: | Purpose:  Cleanse 1CP Taiwan data.                                        |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\twn\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\twn\cleanse
    @set TS_WORKAREA=%TWN_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_TWN.DAT" (
      @echo TWN cleanse input file is missing.  No TWN cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_TWN.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_TWN.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_TWN.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_TWN.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_twtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\apparse"   "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_twcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\appmatch"  "%TS_SETTINGS%\pmatch_p3.stx"      "%TS_LOGS%\p3_twpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\twlatlon_p8.stx" "%TS_LOGS%\p8_twlatlon" "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p4.stx"  "%TS_LOGS%\p4_twdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p5.stx"      "%TS_LOGS%\p5_twwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"    "%TS_LOGS%\p6_twtranfrmr"  "%ERRLOG%"    ))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_TWN.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_TWN.DAT
   ::


   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"  "%TS_LOGS%\p7_twtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSTaiwanCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSTaiwanCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %TWN_WRK%\data\TSS_WINKEY_OUTPUT_TWN.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_TWN.DAT
)
