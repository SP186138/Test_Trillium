...
:: +---------------------------------------------------------------------------+
:: | File:     TSSChinaCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP China data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\chn\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\chn\cleanse
    @set TS_WORKAREA=%CHN_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_CHN.DAT" (
      @echo CHN cleanse input file is missing.  No CHN cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_CHN.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_CHN.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_CHN.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_CHN.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_cntranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\apparse"   "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_cncusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\appmatch"  "%TS_SETTINGS%\pmatch_p3.stx"      "%TS_LOGS%\p3_cnpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\cnlatlon_p8.stx" "%TS_LOGS%\p8_cnlatlon" "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p4.stx"  "%TS_LOGS%\p4_cndrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p5.stx"      "%TS_LOGS%\p5_cnwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"    "%TS_LOGS%\p6_cntranfrmr"  "%ERRLOG%"    ))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_CHN.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_CHN.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"  "%TS_LOGS%\p7_cntranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSChinaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSChinaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %CHN_WRK%\data\TSS_WINKEY_OUTPUT_CHN.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_CHN.DAT
)
