...
:: | File:     TSSLatAmCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP LatAm data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
:: | Client:   Teradata Corporation                                            |
:: | Project:  1 Consumer Place                                                |
:: | Country:  All Latin America                                               |
:: | Author:   Sheldon Hartling                                                |
:: | Date:     16-May-2012                                                     |
:: |                                                                           |
:: | Usage:                                                                    |
:: |   > TSSLatAmCleanse [workarea]                                            |
:: |     where:                                                                |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\lat\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\lat\cleanse
   @set TS_WORKAREA=%LAT_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_LA.DAT" (
      @echo Latin-America cleanse input file is missing.  No Latin-America cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_LA.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_LA.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_LA.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_LA.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_latranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_lacusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p3.stx"  "%TS_LOGS%\p3_ladrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p4.stx"      "%TS_LOGS%\p4_lawinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p5.stx"    "%TS_LOGS%\p5_latranfrmr"  "%ERRLOG%"    ))))
   ...
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"  "%TS_LOGS%\p6_latranfrmr"  "%ERRLOG%" 
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSLatAmCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSLatAmCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
REM copy %LAT_WRK%\data\TSS_WINKEY_OUTPUT_LA.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_LA.DAT
