:: +---------------------------------------------------------------------------+
:: | File:     TSSArabiaMatch.cmd                                          |
:: | Purpose:  Match 1CP Arabia data.                                      | 
:: +---------------------------------------------------------------------------+
:: |                                                                           |
:: | Client:   Teradata Corporation                                            |
:: | Project:  1 Consumer Place                                                |
:: | Country:  Arabia                                                      |
:: | Author:   Debashish Swain                                                     |
:: | Date:     12-Feb-2014                                                     |
:: |                                                                           |
:: | Usage:                                                                    |
:: |   > TSSArabiaMatch [workarea]                                         |
:: |     where:                                                                |
...
:: +---------------------------------------------------------------------------+
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\arb\match
   @set TS_WORKAREA=%ARB_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_ARB.DAT" (
      @echo ERROR: [%DATE%%TIME%] ARB reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_ARB.DAT" (
      @echo ARB cleanse output file is missing.  No ARB matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_ARB.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_ARB.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_ARB (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\lu_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\lu_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\lu_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\lu_srtforrl_p15.dat"  2>nul
   ...
   @del /Q "%TS_DATA%\lu_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\lu_srtforrl_p10.dat" 2>nul
   ::    +---------------------------------------------------------------------+
   ...
   @del /Q "%TS_DATA%\lu_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\lu_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_lu_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_lu_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\lu_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\lu_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\lu_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\lu_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\lu_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\lu_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSArabiaMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSArabiaMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %ARB_WRK%\data\TSS_MATCH_OUTPUT_ARB.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_ARB.DAT
