...
:: | File:     TSSUSACleanse.cmd                                               |
:: | Purpose:  Cleanse 1CP United States data.                                 |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |  ...........  .........  ...............................................  |
:: |  30-Apr-2012  sheldon    Created.                                         |
:: |  07-May-2012  sheldon    Modified to return specific errorlevel values.   |
:: |  08-Apr-2012  sheldon    Phase-0 release.                                 |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\usa\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\usa\cleanse
   @set TS_WORKAREA=%US_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_USA.DAT" (
      @echo USA cleanse input file is missing.  No USA cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_USA.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_USA.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_USA.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_USA.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_ustranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_uscusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\srtforpm_p3.stx"    "%TS_LOGS%\p3_ussrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\uspmatch"  "%TS_SETTINGS%\pmatch_p4.stx"      "%TS_LOGS%\p4_uspmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p5.stx"  "%TS_LOGS%\p5_usdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p6.stx"      "%TS_LOGS%\p6_uswinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"    "%TS_LOGS%\p7_ustranfrmr"  "%ERRLOG%"    ))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_USA.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_USA.DAT
   ::
   
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p8.stx"  "%TS_LOGS%\p8_ustranfrmr"  "%ERRLOG%" 
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSUSACleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSUSACleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %US_WRK%\data\TSS_WINKEY_OUTPUT_USA.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_USA.DAT
)
