:: +---------------------------------------------------------------------------+
:: | File:     TSSSingaporeCleanse.cmd                                         |
:: | Purpose:  Cleanse 1CP Singapore data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\sgp\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\sgp\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_SGP.DAT" (
      @echo SGP cleanse input file is missing.  No SGP cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_SGP.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_SGP.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_SGP.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_SGP.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_sgtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_sgcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\srtforpm_p3.stx"    "%TS_LOGS%\p3_sgsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tqpmatch"  "%TS_SETTINGS%\pmatch_p4.stx"      "%TS_LOGS%\p4_sgpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\sglatlon_p9.stx" "%TS_LOGS%\p9_sglatlon" "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p5.stx"  "%TS_LOGS%\p5_sgdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p6.stx"      "%TS_LOGS%\p6_sgwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"    "%TS_LOGS%\p7_sgtranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_SGP.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_SGP.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p8.stx"  "%TS_LOGS%\p8_sgtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSingaporeCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSingaporeCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %SGP_WRK%\data\TSS_WINKEY_OUTPUT_SGP.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_SGP.DAT
)
