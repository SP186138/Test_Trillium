:: +---------------------------------------------------------------------------+
:: | File:     TSSArabiaCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Arabia data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Debashish\PnG\Trillium\Projects\Arabia\Cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\arb\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_ARB.DAT" (
      @echo ARB cleanse input file is missing.  No ARB cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_ARB.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_ARB.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_ARB.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_ARB.DAT
   ::
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSArabiaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSArabiaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %ARB_WRK%\data\TSS_WINKEY_OUTPUT_ARB.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_ARB.DAT
   )
