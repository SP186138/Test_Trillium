...
:: | File:     TSSIrelandCleanse.cmd                                           |
:: | Purpose:  Cleanse 1CP Ireland data.                                       |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\irl\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\irl\cleanse
   @set TS_WORKAREA=%IRL_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_IRL.DAT" (
      @echo IRL cleanse input file is missing.  No IRL cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_IRL.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_IRL.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Sort for Postal Matcherr
   ::            4: Postal Matcher         
   ::            5: Transformer 2 (data recconstructor)
   ::            6: Geocoder (lat/long)    
   ::            7: Window Key Generator   
   ::            8: Transformer 3 (output)    
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_IRL.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_IRL.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ietranfrmr_p1.stx"  "%TS_LOGS%\p1_ietranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\iecusparse_p2.stx"  "%TS_LOGS%\p2_iecusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\iesrtforpm_p3.stx"  "%TS_LOGS%\p3_iesrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gapmatch"  "%TS_SETTINGS%\iepmatch_p4.stx"    "%TS_LOGS%\p4_iepmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\iedrtrans_p5.stx"   "%TS_LOGS%\p5_iedrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\ielatlon_p9.stx"    "%TS_LOGS%\p9_ielatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\iewinkey_p6.stx"    "%TS_LOGS%\p6_iewinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ietranfrmr_p7.stx"  "%TS_LOGS%\p7_ietranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_IRL.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_IRL.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\ietranfrmr_p8.stx"  "%TS_LOGS%\p8_tranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSIrelandCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSIrelandCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %IRL_WRK%\data\TSS_WINKEY_OUTPUT_IRL.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_IRL.DAT
)
