...
:: +---------------------------------------------------------------------------+
:: | File:     TSSFranceMatch.cmd                                              |
:: | Purpose:  Match 1CP France data.                                          |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\fra\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\fra\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_FRA.DAT" (
      @echo ERROR: [%DATE%%TIME%] FRA reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_FRA.DAT" (
      @echo FRA cleanse output file is missing.  No FRA matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_FRA.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_FRA.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_FRA (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\fr_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\fr_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\fr_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\fr_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\fr_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\fr_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\fr_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\fr_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_fr_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_fr_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\fr_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\fr_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\fr_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\fr_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\fr_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\fr_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSFranceMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSFranceMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %FRA_WRK%\data\TSS_MATCH_OUTPUT_FRA.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_FRA.DAT
