...
:: +---------------------------------------------------------------------------+
:: | File:     TSSKoreaCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP Korea data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\kor\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\kor\cleanse
    @set TS_WORKAREA=%KOR_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_KOR.DAT" (
      @echo KOR cleanse input file is missing.  No KOR cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_KOR.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_KOR.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_KOR.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_KOR.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_krtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\apparse"   "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_krcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\appmatch"  "%TS_SETTINGS%\pmatch_p3.stx"      "%TS_LOGS%\p3_krpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\krlatlon_p8.stx"  "%TS_LOGS%\p8_krlatlon"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p4.stx"  "%TS_LOGS%\p4_krdrtrans"   "%ERRLOG%" | ( 
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p5.stx"      "%TS_LOGS%\p5_krwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"    "%TS_LOGS%\p6_krtranfrmr"  "%ERRLOG%"    ))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_KOR.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_KOR.DAT
   ::


   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"  "%TS_LOGS%\p7_krtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSKoreaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSKoreaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %KOR_WRK%\data\TSS_WINKEY_OUTPUT_KOR.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_KOR.DAT
)
