...
:: | File:     TSSCanadaCleanse.cmd                                            |
:: | Purpose:  Cleanse 1CP Canada data.                                        |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\can\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\can\cleanse
   @set TS_WORKAREA=%CAN_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_CAN.DAT" (
      @echo CAN cleanse input file is missing.  No CAN cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_CAN.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_CAN.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_CAN.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_CAN.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\catranfrmr_p1.stx"  "%TS_LOGS%\p1_catranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cacusparse_p2.stx"  "%TS_LOGS%\p2_cacusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\casrtforpm_p3.stx"  "%TS_LOGS%\p3_casrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\capmatch"  "%TS_SETTINGS%\capmatch_p4.stx"    "%TS_LOGS%\p4_capmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\calatlon_p9.stx"  "%TS_LOGS%\p9_calatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cadrtrans_p5.stx"   "%TS_LOGS%\p5_cadrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\cawinkey_p6.stx"    "%TS_LOGS%\p6_cawinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\catranfrmr_p7.stx"  "%TS_LOGS%\p7_catranfrmr"  "%ERRLOG%"    )))))))
   @call :check_status "p1,p2,p3,p4,p9,p5,p6,p7" "Cleanse"
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_CAN.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_CAN.DAT
   ::
   
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\catranfrmr_p8.stx"  "%TS_LOGS%\p8_catranfrmr"  "%ERRLOG%" 
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCanadaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCanadaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %CAN_WRK%\data\TSS_WINKEY_OUTPUT_CAN.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_CAN.DAT
)
