...
:: | File:     TSSChileCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP Chile data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\chl\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\chl\cleanse
   @set TS_WORKAREA=%CHL_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_CHL.DAT" (
      @echo CHL cleanse input file is missing.  No CHL cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_CHL.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_CHL.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_CHL.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_CHL.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cltranfrmr_p1.stx"  "%TS_LOGS%\p1_cltranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\clcusparse_p2.stx"  "%TS_LOGS%\p2_clcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cldrtrans_p3.stx"   "%TS_LOGS%\p3_cldrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\clwinkey_p4.stx"    "%TS_LOGS%\p4_clwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cltranfrmr_p5.stx"  "%TS_LOGS%\p5_cltranfrmr"  "%ERRLOG%"    ))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_CHL.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_CHL.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cltranfrmr_p6.stx"  "%TS_LOGS%\p6_cltranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSChileCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSChileCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
REM copy %CHL_WRK%\data\TSS_WINKEY_OUTPUT_CHL.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_CHL.DAT
