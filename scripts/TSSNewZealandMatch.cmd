...
:: +---------------------------------------------------------------------------+
:: | File:     TSSNewZealandMatch.cmd                                          |
:: | Purpose:  Match 1CP New Zealand data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_NZL.DAT" (
      @echo ERROR: [%DATE%%TIME%] NZL reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_NZL.DAT" (
      @echo NZL cleanse output file is missing.  No NZL matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_NZL.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_NZL.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_NZL (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\nz_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\nz_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nz_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\nz_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nz_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\nz_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nz_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\nz_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_nz_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_nz_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nz_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\nz_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\nz_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nz_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\nz_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\nz_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNew ZealandMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNew ZealandMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %NZL_WRK%\data\TSS_MATCH_OUTPUT_NZL.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_NZL.DAT
