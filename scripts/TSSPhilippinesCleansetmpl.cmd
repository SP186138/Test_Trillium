:: +---------------------------------------------------------------------------+
:: | File:     TSSPhilippinesCleanse.cmd                                       |
:: | Purpose:  Cleanse 1CP Philippines data.                                   |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |                                                                           |
:: | Usage:                                                                    |
:: |   > TSSPhilippinesCleanse [workarea]                                      |
:: |     where:                                                                |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\phl\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\phl\cleanse
    @set TS_WORKAREA=%PHL_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_PHL.DAT" (
      @echo PHL cleanse input file is missing.  No PHL cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_PHL.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_PHL.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_PHL.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_PHL.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_phtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_phcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"   "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\phlatlon_p7.stx"    "%TS_LOGS%\p7_phlatlon"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p3.stx"  "%TS_LOGS%\p3_phdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p4.stx"      "%TS_LOGS%\p4_phwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p5.stx"    "%TS_LOGS%\p5_phtranfrmr"  "%ERRLOG%"    )))))
   @call :check_status "p1,p2,p7,p3,p4,p5" "Cleanse"
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_PHL.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_PHL.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"  "%TS_LOGS%\p6_phtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSPhilippinesCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSPhilippinesCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %PHL_WRK%\data\TSS_WINKEY_OUTPUT_PHL.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_PHL.DAT
)
