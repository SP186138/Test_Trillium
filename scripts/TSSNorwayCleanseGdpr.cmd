...
:: | File:     TSSNorwayCleanse.cmd                                            |
:: | Purpose:  Cleanse 1CP Norway data.                                        |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_NOR.DAT" (
      @echo NOR cleanse input file is missing.  No NOR cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_NOR.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_NOR.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_NOR.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_NOR.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\notranfrmr_p1.stx"  "%TS_LOGS%\p1_notranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\nocusparse_p2.stx"  "%TS_LOGS%\p2_nocusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\nosrtforpm_p3.stx"  "%TS_LOGS%\p3_nosrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gapmatch"  "%TS_SETTINGS%\nopmatch_p4.stx"    "%TS_LOGS%\p4_nopmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\nodrtrans_p5.stx"   "%TS_LOGS%\p5_nodrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\nolatlon_p9.stx"    "%TS_LOGS%\p9_nolatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\nowinkey_p6.stx"    "%TS_LOGS%\p6_nowinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\notranfrmr_p7.stx"  "%TS_LOGS%\p7_notranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_NOR.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_NOR.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\notranfrmr_p8.stx"  "%TS_LOGS%\p8_notranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNorwayCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNorwayCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %NOR_WRK%\data\TSS_WINKEY_OUTPUT_NOR.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_NOR.DAT
)
