...
:: | File:     TSSMexicoCleanse.cmd                                            |
:: | Purpose:  Cleanse 1CP Mexico data.                                        |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\mex\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\mex\cleanse
   @set TS_WORKAREA=%MEX_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_MEX.DAT" (
      @echo MEX cleanse input file is missing.  No MEX cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_MEX.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_MEX.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_MEX.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_MEX.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\mxtranfrmr_p1.stx"  "%TS_LOGS%\p1_mxtranfrmr"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\mxcusparse_p2.stx"  "%TS_LOGS%\p2_mxcusparse"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\mxdrtrans_p5.stx"   "%TS_LOGS%\p5_mxdrtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\mxwinkey_p6.stx"    "%TS_LOGS%\p6_mxwinkey"      "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\mxtranfrmr_p7.stx"  "%TS_LOGS%\p7_mxtranfrmr"    "%ERRLOG%"    ))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_MEX.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_MEX.DAT
   ::
   
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\mxtranfrmr_p8.stx"  "%TS_LOGS%\p8_mxtranfrmr"  "%ERRLOG%" 
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSMexicoCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSMexicoCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
REM copy %MEX_WRK%\data\TSS_WINKEY_OUTPUT_MEX.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_MEX.DAT
