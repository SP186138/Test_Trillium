:: +---------------------------------------------------------------------------+
:: | File:     TSSTurkeyMatch.cmd                                              |
:: | Purpose:  Match 1CP Turkey data.                                          |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\tur\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\tur\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_TUR.DAT" (
      @echo ERROR: [%DATE%%TIME%] TUR reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_TUR.DAT" (
      @echo TUR cleanse output file is missing.  No TUR matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_TUR.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_TUR.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_TUR (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\tr_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\tr_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\tr_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\tr_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\tr_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\tr_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\tr_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\tr_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_tr_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_tr_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\tr_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\tr_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\tr_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\tr_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\tr_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\tr_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSTurkeyMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSTurkeyMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %TRK_WRK%\data\TSS_MATCH_OUTPUT_TUR.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_TUR.DAT
