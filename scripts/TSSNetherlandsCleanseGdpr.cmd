...
:: | File:     TSSNetherlandsCleanse.cmd                                       |
:: | Purpose:  Cleanse 1CP Netherlands data.                                   |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |                                                                           |
:: | Usage:                                                                    |
:: |   > TSSNetherlandsCleanse [workarea]                                      |
:: |     where:                                                                |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\nld\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\nld\cleanse
   @set TS_WORKAREA=%NLD_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_NLD.DAT" (
      @echo NLD cleanse input file is missing.  No NLD cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_NLD.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_NLD.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_NLD.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_NLD.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\nltranfrmr_p1.stx"  "%TS_LOGS%\p1_nltranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\nlcusparse_p2.stx"  "%TS_LOGS%\p2_nlcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\nlsrtforpm_p3.stx"  "%TS_LOGS%\p3_nlsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tqpmatch"  "%TS_SETTINGS%\nlpmatch_p4.stx"    "%TS_LOGS%\p4_nlpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\nldrtrans_p5.stx"   "%TS_LOGS%\p5_nldrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\nllatlon_p9.stx"    "%TS_LOGS%\p9_nllatlon"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\nlwinkey_p6.stx"    "%TS_LOGS%\p6_nlwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\nltranfrmr_p7.stx"  "%TS_LOGS%\p7_nltranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_NLD.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_NLD.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\nltranfrmr_p8.stx"  "%TS_LOGS%\p8_nltranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNetherlandsCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNetherlandsCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %NLD_WRK%\data\TSS_WINKEY_OUTPUT_NLD.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_NLD.DAT
)
