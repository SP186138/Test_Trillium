...
:: +---------------------------------------------------------------------------+
:: | File:     TSSOtherAsiaCleanse.cmd                                         |
:: | Purpose:  Cleanse 1CP OtherAsia data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\zzz\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\zzz\cleanse
    @set TS_WORKAREA=%OTH_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_OTH.DAT" (
      @echo ZZZ cleanse input file is missing.  No ZZZ cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_OTH.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_OTH.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_OTH.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_OTH.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"     "%TS_LOGS%\p1_zztranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"     "%TS_LOGS%\p2_zzcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\zz_tranfrmr_p3.stx"  "%TS_LOGS%\p3_zztranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\srtforpm_p4.stx"     "%TS_LOGS%\p4_zzsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gapmatch"  "%TS_SETTINGS%\pmatch_p5.stx"       "%TS_LOGS%\p5_zzpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"   "%TSQ_BIN%\gllatlon"  "%TS_SETTINGS%\zzlatlon_p10.stx"    "%TS_LOGS%\p10_zzlatlon"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p6.stx"   "%TS_LOGS%\p6_zzdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p7.stx"       "%TS_LOGS%\p7_zzwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p8.stx"     "%TS_LOGS%\p8_zztranfrmr"  "%ERRLOG%"    ))))))))
   @call :check_status "p1,p2,p3,p4,p5,p10,p6,p7,p8" "Cleanse"
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_OTH.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_OTH.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p9.stx"  "%TS_LOGS%\p9_zztranfrmr"  "%ERRLOG%"
   @call :check_status "p9" "CreateTriggerFile"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSOtherAsiaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSOtherAsiaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %OTH_WRK%\data\TSS_WINKEY_OUTPUT_OTH.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_OTH.DAT
)
