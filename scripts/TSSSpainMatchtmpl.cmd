:: +---------------------------------------------------------------------------+
:: | File:     TSSSpainMatch.cmd                                               |
:: | Purpose:  Match 1CP Spain data.                                           |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |  01-Aug-2013  sheldon    New multi-match design.                          |
:: |  01-Jan-2014  sheldon    Upgraded to TSS v14.0.                           |
:: |  07-Mar-2014  sheldon    Added 'NationalId Trumps All' transformer.       |
:: |                                                                           |
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\esp\match
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\esp\match
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_ESP.DAT" (
      @echo ERROR: [%DATE%%TIME%] ESP reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_ESP.DAT" (
      @echo ESP cleanse output file is missing.  No ESP matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_ESP.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_ESP.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_ESP (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\es_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\es_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\es_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p10.dat" 2>nul

   ::    +---------------------------------------------------------------------+
   ::... | Match 4: National ID Number                                         |
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  ("tmp1")
   ::            ("maout1")
   ...
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"  "%TS_SETTINGS%\essrtforrl_p25.stx"  "%TS_LOGS%\p25_essrtforrl"  "%ERRLOG%"
   @call :check_status "p25" "SortRefFileByNationalId"
   @if %status% == 1 (set status=551 & goto end)

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"  "%TS_SETTINGS%\essrtforrl_p26.stx"  "%TS_LOGS%\p26_essrtforrl"  "%ERRLOG%"
   @call :check_status "p26" "SortTrnFileByNationalId"
   @if %status% == 1 (set status=552 & goto end)

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\rellink"  "%TS_SETTINGS%\esrellinkref_p27.stx"  "%TS_LOGS%\p27_esrellinkref"  "%ERRLOG%"
   @call :check_status "p27" "Match#4ByNationalId"
   @if %status% == 1 (set status=553 & goto end)

   :: OK to delete tmp5 & tmp8
   @del /Q "%TS_DATA%\es_srtforrl_p25.dat" 2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p26.dat" 2>nul
   ...
   @call :check_status "p17" "CombineMatch#2#3#4AndLinks"
   @if %status% == 1 (set status=602 & goto end)
   @call :check_status "p18" "ResolveTransitivity"
   @if %status% == 1 (set status=602 & goto end)

   :: OK to delete maout2, maout3, maout4, link2, link3 & link4
   @del /Q "%TS_DATA%\es_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\es_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\es_rellinkref_p27.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_es_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_es_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_es_rellinkref_p27.dat" 2>nul
   ...
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p28.stx"  "%TS_LOGS%\p28_tranfrmr"  "%ERRLOG%" | (
   ...
   @call :check_status "p28" "TranfrmrNationalIdTrumpsAll"
   @if %status% == 1 (set status=603 & goto end)
   @call :check_status "p20" "SortMatchedHouseholdsById"
   @if %status% == 1 (set status=603 & goto end)

   :: OK to delete tmp9, tmp10 & maout1
   @del /Q "%TS_DATA%\es_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\es_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\es_resolve_p18.dat" 2>nul
   ...
   @call :check_status "p21" "Match#5Individuals"
   @if %status% == 1 (set status=702 & goto end)
   @call :check_status "p22" "WriteFinalOutputFile"
   @if %status% == 1 (set status=702 & goto end)

   :: OK to delete tmp1, tmp2 & tmp11
   @del /Q "%TS_DATA%\es_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\es_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSpainMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSSpainMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %ESP_WRK%\data\TSS_MATCH_OUTPUT_ESP.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_ESP.DAT
