...
:: +---------------------------------------------------------------------------+
:: | File:     TSSNetherlandsMatch.cmd                                         |
:: | Purpose:  Match 1CP Netherlands data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   @if NOT EXIST "%TS_DATA%\TSS_MATCH_INPUT_CRM_NLD.DAT" (
      @echo ERROR: [%DATE%%TIME%] NLD reference CRM file is missing.
      @set status=202
      @goto end
   )
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_OUTPUT_NLD.DAT" (
      @echo NLD cleanse output file is missing.  No NLD matching performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_MATCH_OUTPUT_NLD.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_MATCH_INPUT_CRM_NLD.DAT.
   ::    Writes: ("tmp1")
   ::
   ...
   ::
   ::    Reads:  ("tmp1")
   ::            TSS_CLEANSE_OUTPUT_NLD (output from cleanse)
   ::    Writes: ("tmp6")
   ::            ("tmp3")
   ::            ("maout1")
   ::
   ...
   @del /Q "%TS_DATA%\nl_srtforrl_p7.dat" 2>nul
   @del /Q "%TS_DATA%\nl_srtforrl_p2.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nl_srtforrl_p13.dat" 2>nul
   @del /Q "%TS_DATA%\nl_srtforrl_p15.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nl_srtforrl_p11.dat" 2>nul
   @del /Q "%TS_DATA%\nl_srtforrl_p10.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nl_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\nl_rellinkref_p12.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_nl_rellinkref_p16.dat" 2>nul
   @del /Q "%TS_DATA%\LINKING_nl_rellinkref_p12.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nl_tranfrmr_p9.dat" 2>nul
   @del /Q "%TS_DATA%\nl_tsqsort_p14.dat" 2>nul
   @del /Q "%TS_DATA%\nl_resolve_p18.dat" 2>nul
   ...
   @del /Q "%TS_DATA%\nl_tranfrmr_p4.dat"  2>nul
   @del /Q "%TS_DATA%\nl_srtforrl_p6.dat"  2>nul
   @del /Q "%TS_DATA%\nl_srtforrl_p20.dat" 2>nul
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNetherlandsMatch job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNetherlandsMatch job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
...
copy %NLD_WRK%\data\TSS_MATCH_OUTPUT_NLD.DAT %TSS_OUTPUT%\TSS_MATCH_OUTPUT_NLD.DAT
