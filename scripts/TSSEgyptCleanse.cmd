...
:: +---------------------------------------------------------------------------+
:: | File:     TSSEgyptCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Egypt data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\egy\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\egy\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_EGY.DAT" (
      @echo EGY cleanse input file is missing.  No EGY cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_EGY.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_EGY.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_EGY.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_EGY.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\egtranfrmr_p1.stx"  "%TS_LOGS%\p1_egtranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\egdrtrans_p2.stx"   "%TS_LOGS%\p2_egdrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\egtranfrmr_p3.stx"  "%TS_LOGS%\p3_egtranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_EGY.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_EGY.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\egtranfrmr_p4.stx"  "%TS_LOGS%\p4_egtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSEgyptCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSEgyptCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %EGY_WRK%\data\TSS_WINKEY_OUTPUT_EGY.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_EGY.DAT
)
