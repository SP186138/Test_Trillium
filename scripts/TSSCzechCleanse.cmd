...
:: +---------------------------------------------------------------------------+
:: | File:     TSSCzechCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Czech data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\cze\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\cze\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_CZE.DAT" (
      @echo CZE cleanse input file is missing.  No CZE cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_CZE.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_CZE.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_CZE.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_CZE.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cztranfrmr_p1.stx"  "%TS_LOGS%\p1_cztranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\czdrtrans_p2.stx"   "%TS_LOGS%\p2_czdrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cztranfrmr_p3.stx"  "%TS_LOGS%\p3_cztranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_CZE.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_CZE.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\cztranfrmr_p4.stx"  "%TS_LOGS%\p4_cztranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCzechCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSCzechCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %CZE_WRK%\data\TSS_WINKEY_OUTPUT_CZE.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_CZE.DAT
)
