...
:: +---------------------------------------------------------------------------+
:: | File:     TSSBulgariaCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP Bulgaria data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\bgr\cleanse
   ...
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\bgr\cleanse
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_BGR.DAT" (
      @echo BGR cleanse input file is missing.  No BGR cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_BGR.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_BGR.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_BGR.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_BGR.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\bgtranfrmr_p1.stx"  "%TS_LOGS%\p1_bgtranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\bgdrtrans_p2.stx"   "%TS_LOGS%\p2_bgdrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\bgtranfrmr_p3.stx"  "%TS_LOGS%\p3_bgtranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_BGR.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_BGR.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\bgtranfrmr_p4.stx"  "%TS_LOGS%\p4_bgtranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSBulgariaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSBulgariaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %BGR_WRK%\data\TSS_WINKEY_OUTPUT_BGR.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_BGR.DAT
)
