:: +---------------------------------------------------------------------------+
:: | File:     TSSJapanCleanse.cmd                                             |
:: | Purpose:  Cleanse 1CP Japan data.                                         |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\jpn\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\jpn\cleanse
    @set TS_WORKAREA=%JPN_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_JPN.DAT" (
      @echo JPN cleanse input file is missing.  No JPN cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_JPN.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_JPN.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Postal Matcher
   ::            4: Transformer 2 (data reconstructor)
   ::            5: Window Key Generator
   ::            6: Transformer 3 (output)
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_JPN.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_JPN.DAT
   ::
   
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_jptranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\apparse"   "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_jpcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\appmatch"  "%TS_SETTINGS%\pmatch_p3.stx"      "%TS_LOGS%\p3_jppmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\jplatlon_p8.stx" "%TS_LOGS%\p8_jplatlon" "%ERRLOG%" | ( 
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p4.stx"  "%TS_LOGS%\p4_jpdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p5.stx"      "%TS_LOGS%\p5_jpwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"    "%TS_LOGS%\p6_jptranfrmr"  "%ERRLOG%"    ))))))
   @call :check_status "p1,p2,p3,p8,p4,p5,p6" "Cleanse"
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_JPN.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_JPN.DAT
   ::


   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"  "%TS_LOGS%\p7_jptranfrmr"  "%ERRLOG%"
   @call :check_status "p7" "CreateTriggerFile"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSJapanCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSJapanCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %JPN_WRK%\data\TSS_WINKEY_OUTPUT_JPN.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_JPN.DAT
)
