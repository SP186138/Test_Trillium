...
set TSS_SCRIPTS=\\10.36.32.37\scripts_dev
set T_SCRIPTS=E:\scripts_dev
set TSS_PROJECTS=E:\projects_dev
...
set MDM_CUSTOM_DIR=\\10.36.32.35\Teradata\MDM\3.00.02_SIT2\custom\pngrelease3
...
:: | File:     TSSHungaryCleanse.cmd                                           |
:: | Purpose:  Cleanse 1CP Hungary data.                                       |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\hun\cleanse
   ...
   @set TS_QUALITY=%TSS_SWE%\Software
   @set TS_POSTAL=%TS_QUALITY%\..\tables
   @set TS_PROJECT=%TSS_PROJECTS%\hun\cleanse
   @set TS_WORKAREA=%HUN_WRK%
   @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_HUN.DAT" (
      @echo HUN cleanse input file is missing.  No HUN cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_HUN.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_HUN.DAT"
      @set status=0
      @goto end
   )
   ...
   ::            3: Transformer 3
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_HUN.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_HUN.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hutranfrmr_p1.stx"  "%TS_LOGS%\p1_hutranfrmr"  "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hudrtrans_p2.stx"   "%TS_LOGS%\p2_hudrtrans"   "%ERRLOG%" | (
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hutranfrmr_p3.stx"  "%TS_LOGS%\p3_hutranfrmr"  "%ERRLOG%"    ))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_HUN.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_HUN.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hutranfrmr_p4.stx"  "%TS_LOGS%\p4_hutranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHungaryCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHungaryCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %HUN_WRK%\data\TSS_WINKEY_OUTPUT_HUN.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_HUN.DAT
)
