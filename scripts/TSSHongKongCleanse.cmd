...
:: +---------------------------------------------------------------------------+
:: | File:     TSSHongKongCleanse.cmd                                          |
:: | Purpose:  Cleanse 1CP Hong Kong data.                                     |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: |  ...........  .........  ...............................................  |
:: |  18-May-2012  sheldon    Created.                                         |
:: |  22-May-2012  sheldon    Updated to add initial transformer.              |
:: |  23-May-2012  sheldon    Corrected file references from USA/BRA to HKG.   |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\hkg\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\hkg\cleanse
    @set TS_WORKAREA=%HKG_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_HKG.DAT" (
      @echo HKG cleanse input file is missing.  No HKG cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_HKG.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_HKG.DAT"
      @set status=0
      @goto end
   )
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_HKG.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_HKG.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\hktranfrmr_p13.stx"  "%TS_LOGS%\p13_hktranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\mrgsplit"  "%TS_SETTINGS%\mrgsplit_p1.stx"     "%TS_LOGS%\p1_hkmrgsplit"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p6.stx"     "%TS_LOGS%\p6_hktranfrmr"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p7.stx"     "%TS_LOGS%\p7_hkcusparse"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\hkpmatch"  "%TS_SETTINGS%\pmatch_p8.stx"       "%TS_LOGS%\p8_hkpmatch"     "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\hklatlon_p14.stx" "%TS_LOGS%\p14_hklatlon" "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p9.stx"   "%TS_LOGS%\p9_hkdrtrans"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p10.stx"      "%TS_LOGS%\p10_hkwinkey"    "%ERRLOG%"    )))))))
   @call :check_status "p13,p1,p6,p7,p8,p14,p9,p10" "CleanseSingleByte"
   @if %status% == 1 (set status=112 & goto end)

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p2.stx"    "%TS_LOGS%\p2_hktranfrmr"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\busparse"  "%TS_SETTINGS%\busparse_p3.stx"    "%TS_LOGS%\p3_hkbusparse"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p4.stx"  "%TS_LOGS%\p4_hkdrtranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p5.stx"      "%TS_LOGS%\p5_hkwinkey"      "%ERRLOG%" )))
   @call :check_status "p2,p3,p4,p5" "CleanseDoubleByte"
   @if %status% == 1 (set status=112 & goto end)
   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p11.stx"   "%TS_LOGS%\p11_hktranfrmr"   "%ERRLOG%"   
   @call :check_status "p11" "CleanseOutput"
   @if %status% == 1 (set status=112 & goto end)

   :: OK to delete tmp files
   @del /Q "%TS_DATA%\hk_mrgsplit_p1.dat.002" 2>nul
   @del /Q "%TS_DATA%\hk_winkey_p10.dat"      2>nul
   
   ::    +---------------------------------------------------------------------+
   ::... | Create the WINKEY_OUTPUT file.                                      |
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_HKG.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_HKG.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p12.stx"    "%TS_LOGS%\p12_hktranfrmr"   "%ERRLOG%"
   @call :check_status "p12" "CreateTriggerFile"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHongKongCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSHongKongCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %HKG_WRK%\data\TSS_WINKEY_OUTPUT_HKG.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_HKG.DAT
)
