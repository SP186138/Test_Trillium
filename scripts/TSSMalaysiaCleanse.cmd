...
:: +---------------------------------------------------------------------------+
:: | File:     TSSMalaysiaCleanse.cmd                                          |
:: | Purpose:  Cleanse 1CP Malaysia data.                                      |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   ::! @set TS_PROJECT=C:\Trillium_Development\v14\development\projects\mys\cleanse
   ...
   :: Teradata P&G server environment
    ...
    @set TS_POSTAL=%TS_QUALITY%\..\tables
    @set TS_PROJECT=%TSS_PROJECTS%\mys\cleanse
    @set TS_WORKAREA=%MYS_WRK%
    @set PATH=%TS_QUALITY%\bin;%SystemRoot%\system32;%SystemRoot%
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_MYS.DAT" (
      @echo MYS cleanse input file is missing.  No MYS cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_MYS.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_MYS.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_MYS.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_MYS.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_mytranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_mycusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\srtforpm_p3.stx"    "%TS_LOGS%\p3_mysrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tqpmatch"  "%TS_SETTINGS%\pmatch_p4.stx"      "%TS_LOGS%\p4_mypmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd" "%TSQ_BIN%\gllatlon" "%TS_SETTINGS%\mylatlon_p9.stx" "%TS_LOGS%\p9_mylatlon" "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p5.stx"  "%TS_LOGS%\p5_mydrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p6.stx"      "%TS_LOGS%\p6_mywinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"    "%TS_LOGS%\p7_mytranfrmr"  "%ERRLOG%"    )))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_MYS.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_MYS.DAT
   ::

   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p8.stx"  "%TS_LOGS%\p8_mytranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSMalaysiaCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSMalaysiaCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %MYS_WRK%\data\TSS_WINKEY_OUTPUT_MYS.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_MYS.DAT
)
