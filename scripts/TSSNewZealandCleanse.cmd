...
set TSS_SCRIPTS=\\143.30.195.8\scripts
set TSS_SCRIPTS_ASIA=\\143.30.195.3\scripts
set TSS_SCRIPTS_EUROPE=\\143.30.195.7\scripts
set TSS_PROCESS=i2
set RTNTN_DATE=0
...
set MDM_CUSTOM_DIR=\\143.30.195.1\Teradata\MDM\3.04.01.00\custom\pngrelease3
...
set PRETLR=\\143.30.195.3\TrilliumDeployment-RETLR\project7\batch
...
:: +---------------------------------------------------------------------------+
:: | File:     TSSNewZealandCleanse.cmd                                        |
:: | Purpose:  Cleanse 1CP NewZealand data.                                    |
:: +---------------------------------------------------------------------------+
:: |                                                                           |
...
:: +---------------------------------------------------------------------------+
   ...
   @if NOT EXIST "%TS_DATA%\TSS_CLEANSE_INPUT_NZL.DAT" (
      @echo NZL cleanse input file is missing.  No NZL cleansing performed. >>"%RUNLOG%"
      @type nul > "%TS_DATA%\TSS_CLEANSE_OUTPUT_NZL.DAT"
      @type nul > "%TS_DATA%\TSS_WINKEY_OUTPUT_NZL.DAT"
      @set status=0
      @goto end
   )
   ...
   ::
   ::    Reads:  TSS_CLEANSE_INPUT_NZL.DAT
   ::    Writes: TSS_CLEANSE_OUTPUT_NZL.DAT
   ::

   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p1.stx"    "%TS_LOGS%\p1_nztranfrmr"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\cusparse"  "%TS_SETTINGS%\cusparse_p2.stx"    "%TS_LOGS%\p2_nzcusparse"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tsqsort"   "%TS_SETTINGS%\srtforpm_p3.stx"    "%TS_LOGS%\p3_nzsrtforpm"  "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\gapmatch"  "%TS_SETTINGS%\pmatch_p4.stx"      "%TS_LOGS%\p4_nzpmatch"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\drtranfrmr_p5.stx"  "%TS_LOGS%\p5_nzdrtrans"   "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\winkey"    "%TS_SETTINGS%\winkey_p6.stx"      "%TS_LOGS%\p6_nzwinkey"    "%ERRLOG%" | (
   @"%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p7.stx"    "%TS_LOGS%\p7_nztranfrmr"  "%ERRLOG%"    ))))))
   ...
   ::    +---------------------------------------------------------------------+
   ::
   ::    Reads:  TSS_CLEANSE_OUTPUT_NZL.DAT
   ::    Writes: TSS_WINKEY_OUTPUT_NZL.DAT
   ::


   @call "%HOME%\run_process.cmd"  "%TSQ_BIN%\tranfrmr"  "%TS_SETTINGS%\tranfrmr_p8.stx"  "%TS_LOGS%\p8_nztranfrmr"  "%ERRLOG%"
   ...
   @if %status% == 0 (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNewZealandCleanse job succeeded!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo.
   ) else (
      @echo.
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @echo TSSNewZealandCleanse job failed!
      @echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      @if EXIST "%ERRLOG%" (
         @type "%ERRLOG%" 1>&2
   ...
   @if "%TSS_PROCESS%" == "i2" (
copy %NZL_WRK%\data\TSS_WINKEY_OUTPUT_NZL.DAT %TSS_OUTPUT%\TSS_WINKEY_OUTPUT_NZL.DAT
)
